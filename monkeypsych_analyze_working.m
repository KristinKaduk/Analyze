...
% [out_comp variable_to_test counter]= monkeypsych_analyze_working({'W:\Data\Curius_microstim_with_parameters\20140211',2},{'nsacc_max',10,'summary',0,'success',1,'display',1,'saccade_1bo',1,'sac_ini_t',30,'sac_end_t',15,'sac_min_dur',0.01,'only_voluntary_saccades',1})
% [out_comp variable_to_test counter]= monkeypsych_analyze_working({'W:\Data\Curius_microstim_with_parameters\20140211',2},{'summary',1,'display',1,'choice',1,'trial_history_mode',1,'microstim',1,'observed_structure','binary','current_history_parameter','eyetar_l','observed_value',1,'past_parameter','microstim','past_value',1,'consecutive_trials',1,'n_trials_past',5,'past_keys',{'choice',NaN,'microstim',NaN}})
% [out_comp variable_to_test counter]= monkeypsych_analyze_working({'W:\Data\Curius_microstim_with_parameters\20140211',2},{'summary',1,'display',1})
...
% success                   If it was a successful trial
...
% 'success'                         success trial selector                                          1 (success), 0 (error)
% 'reward_modulation'
% 'reward_selected'
% 'abort_code'
...
% 'summary'                         Select what to plot                                             0 (trial by trial), 1 (only first...) 2 (first two...) 3(all three...)  ...summary plots per batch
...
saccades=struct('lat', nanpar1, 'dur', nanpar1, 'velocity', nanpar1, 'num_sac', nanpar1, 'sel_n_sac', nanpar1, 'evoked', nanpar1,'ini_evo',nanpar1,...
    'startpos', nanpar2,'endpos', nanpar2,'accuracy_xy', nanpar2,'accuracy_rad',nanpar2,'tar_pos_closest',nanpar2,...
...
reaches=struct('lat', nanpar1, 'dur', nanpar1, 'demanded_hand', nanpar1, 'reach_hand', nanpar1, 'ini',nanpar1,'end_stat',nanpar1,'ini_fix',nanpar1 ,'dur_fix',nanpar1,...
...
task=struct('type', nanpar1, 'effector', nanpar1,'abort_code', nanpar1,'reward_modulation',nanpar1,'reward_size',nanpar1,'reward_time',nanpar1,...
...
for n = 1:amount_of_selected_trials
    ...
    elseif effector_sr(n,2) && ~isnan(s_i_t_h(n)) && s_i_t_h(n)<=2
        task(n).tar_rew_high            =trial(n).eye.tar(s_i_t_h(n)).reward_time(1);
        task(n).tar_rew_low             =trial(n).eye.tar(s_i_t_h(n)).reward_time(2);
        task(n).tar_rew_prob_high       =trial(n).eye.tar(s_i_t_h(n)).reward_prob;
        if ~isnan(n_i_t_h(n))
            task(n).nct_rew_high        =trial(n).eye.tar(n_i_t_h(n)).reward_time(1);
            task(n).nct_rew_low         =trial(n).eye.tar(n_i_t_h(n)).reward_time(2);
            task(n).nct_rew_prob_high   =trial(n).eye.tar(n_i_t_h(n)).reward_prob;
        end
    end
...
%                    saccades(n).n_1ao               = n_1ao;
        ...
        if isfield(trial(n).eye.tar(1),'shape') && isstruct(trial(n).eye.tar(1).shape)
            if ~isnan(s_i_t_e(n))
...
MA_STATES.ALL               =[MA_STATES.INI_TRI MA_STATES.FIX_ACQ  MA_STATES.FIX_HOL  MA_STATES.CUE_ON  MA_STATES.MEM_PER  MA_STATES.DEL_PER MA_STATES.TAR_ACQ_INV  MA_STATES.TAR_HOL_INV...
    MA_STATES.TAR_ACQ  MA_STATES.TAR_HOL MA_STATES.MAT_ACQ MA_STATES.MAT_HOL MA_STATES.MAT_ACQ_MSK MA_STATES.MAT_HOL_MSK MA_STATES.SEN_RET MA_STATES.SUCCESS_ABORT  MA_STATES.REWARD  MA_STATES.ITI];
MA_STATES.ALL_NAMES         ={'Trial Initiation', 'Fixation Acquisition', 'Fixation Hold', 'Cue On', 'Memory period', 'Delay period', 'Target Acquisition (invisible)',...
    'Target Hold (invisible)','Target Acquisition', 'Target Hold', 'Match Aquisition', 'Match Hold', 'Match Aquisition (Masked)', 'Match Hold (Masked)', 'Return to Sensors', 'Not aborted', 'Reward', 'ITI'};
MA_STATES.LABELS={'INI','Initial acq.', 'Initial hold', 'Cue on', 'Memory ', 'Delay', 'Tar inv acq.', 'Tar inv hold', 'Tar acq.', 'Target hold', 'Tar acq.', 'Tar hold', 'Tar acq. mask', 'Tar hold', 'Sen Ret', 'Success', 'Reward', 'ITI'};
MA_STATES.ALL_CHANGE_NAMES  ={'Holding sensors(?)', 'fixation', 'fixation brightening', 'cue onset', 'cue offset', 'delay period',...
    'go signal', 'target acquired', 'targets visible', 'targets brightening', 'targets visible', 'targets brightening', 'masked targets visible', 'target revealed', 'Target sensor reached', 'success', 'reward', 'ITI'};
...
% function [statistic, correlation] = saccade_reach_correlation(keys,reaches,saccades,task,logidx)
% 
% if nargin<5
%    logidx=struct; 
% end
...
%         [~,statistic(sel).(['sac_' par]).residuals] = regress_ignoring_empties([saccades(idx_sac).(par)]',find(idx_sac)');
%         [~,statistic(sel).(['rea_' par]).residuals] = regress_ignoring_empties([reaches(idx_rea).(par)]',find(idx_rea)');      
%         [~,statistic(sel).(['sac_' par '_cor']).residuals] = regress_ignoring_empties([saccades(idx_sac_rea).(par)]',find(idx_sac_rea)');
%         [~,statistic(sel).(['rea_' par '_cor']).residuals] = regress_ignoring_empties([reaches(idx_sac_rea).(par)]',find(idx_sac_rea)'); 
%         [correlation(sel).([par '_r']),correlation(sel).([par '_sig']),correlation(sel).([par '_slo']),correlation(sel).([par '_int'])] = corr_ignoring_empties([saccades(idx{sel}).(par)],[reaches(idx{sel}).(par)],'Spearman');
%         [correlation(sel).([par '_raw_sac_rea'])] = [[saccades(idx{sel}).(par)]; [reaches(idx{sel}).(par)]];
%         [correlation(sel).([par '_r_residuals']),correlation(sel).([par '_sig_residuals']),correlation(sel).([par '_slo_residuals']),correlation(sel).([par '_int_residuals'])] = corr_ignoring_empties(statistic(sel).(['sac_' par  '_cor']).residuals,statistic(sel).(['rea_' par  '_cor']).residuals,'Spearman');     
%         [correlation(sel).([par '_residuals_sac_rea'])]  = [statistic(sel).(['sac_' par  '_cor']).residuals; statistic(sel).(['rea_' par  '_cor']).residuals];
%         
%     end    
% end
% end
...
%         graded_color(trial(n).time_axis_i >= times.state_changes_x(1,Line_n),3) = state_colors(Line_n,3);
    ...
    newPosition = [0.65 0.6 0.08 0.17]; %x_pos y_pos width height % all in proportion to figure size
    ...
    newPosition = [0.85 0.6 0.08 0.17]; %x_pos y_pos width height % all in proportion to figure size
...
for n=1:numel(trial)
    if isfield(trial(n).(eye_or_hnd).tar, 'pos') %% to debug new monkeypsysch_test
